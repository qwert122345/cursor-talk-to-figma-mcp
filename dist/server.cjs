...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).describe("COMPONENT_SET node ids"),
    from: import_zod.z.string().describe("Current property key, e.g. Size"),
    to: import_zod.z.string().describe("New property key, e.g. Scale"),
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).optional().describe("Explicit node ids to unbind"),
    pageName: import_zod.z.string().optional().describe("Scope the sweep to one page"),
    nodeId: import_zod.z.string().optional().describe("Scope the sweep to this node and its children"),
    excludePages: import_zod.z.array(import_zod.z.string()).optional().describe("Page names to skip when sweeping"),
    onlyOutsideComponents: import_zod.z.boolean().optional().describe("Only nodes with no component ancestor (documentation frames)"),
    property: import_zod.z.string().optional().describe("itemSpacing | counterAxisSpacing | cornerRadius | padding | paddingLeft/Right/Top/Bottom | strokeWeight | width | height"),
    properties: import_zod.z.array(import_zod.z.string()).optional().describe("Several of the above at once, in a single traversal"),
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).optional().describe("Component or component-set ids to read"),
    pageName: import_zod.z.string().optional().describe("Read every top-level component on this page instead")
  },
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).describe("INSTANCE node ids to rebind"),
    toKey: import_zod.z.string().describe("Key of the component to swap to (from get_local_components)"),
    viaNodeId: import_zod.z.string().optional().describe("An instance already bound to the target's component set; the target variant is taken from its siblings. Required for `.`-prefixed components"),
    dryRun: import_zod.z.boolean().optional().describe("Report what would change without writing")
  },
  async ({ nodeIds, toKey, viaNodeId, dryRun }) => {
    try {
...
server.tool(
  ...
  {
    sample: import_zod.z.number().optional().describe("Max consumer nodes listed per style (default 20); counts are always complete")
  },
  async ({ sample }) => {
    try {
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).describe("Nodes to change"),
    opacity: import_zod.z.number().min(0).max(1).describe("Opacity 0-1, e.g. 0.3 for 30%")
  },
...
server.tool(
  ...
  {
    name: import_zod.z.string().describe("Layer name to match (substring, case-insensitive, unless exact)"),
    exact: import_zod.z.boolean().optional().describe("Require the whole name to match exactly, case-sensitive"),
    types: import_zod.z.array(import_zod.z.string()).optional().describe('Only report these node types, e.g. ["FRAME"]'),
    parentTypes: import_zod.z.array(import_zod.z.string()).optional().describe('Only report nodes whose direct parent is one of these types, e.g. ["COMPONENT"]'),
    idsOnly: import_zod.z.boolean().optional().describe("Return only the id array (unpaged) instead of full findings"),
    nodeId: import_zod.z.string().optional().describe("Limit the scan to this node and its children"),
    pageName: import_zod.z.string().optional().describe("Limit the scan to a single page by name"),
    excludePages: import_zod.z.array(import_zod.z.string()).optional().describe("Page names to skip"),
    limit: import_zod.z.number().optional().describe("Max findings to return (default 500)"),
    offset: import_zod.z.number().optional().describe("Findings to skip, for paging")
  },
...
server.tool(
  ...
  `Find nodes that exist in the file but are not visible on canvas, with the reason for each: hidden (visible=false), parentHidden (an ancestor is toggled off), transparent (opacity 0), faded (opacity below 1, only with includeFaded), zeroSize, clipped (fully outside a clipsContent ancestor) or clippedPartly. get_node_info does not return visible/opacity/clipsContent, so this is the only way to answer "why can't I see it". Scope with nodeId or pageName, skip pages with excludePages, and narrow with nameFilter (substring, case-insensitive).`,
  {
    ...
    pageName: import_zod.z.string().optional().describe("Limit the scan to a single page by name"),
    excludePages: import_zod.z.array(import_zod.z.string()).optional().describe("Page names to skip"),
    nameFilter: import_zod.z.string().optional().describe('Only report nodes whose name contains this (case-insensitive), e.g. ".Slot"'),
    includeFaded: import_zod.z.boolean().optional().describe('Also report nodes with opacity below 1 as reason "faded" (detail.opacity holds the value). Off by default because intentionally translucent nodes are common; turn it on to read opacity, which no other command returns.'),
    limit: import_zod.z.number().optional().describe("Max findings to return (default 500)"),
    offset: import_zod.z.number().optional().describe("Findings to skip, for paging")
  },
...
server.tool(
  ...
  "Find nodes that hardcode a property (fills, strokes, cornerRadius, itemSpacing, paddingLeft/Right/Top/Bottom, fontSize) instead of binding a variable, and SELECT them in Figma so a person can review them. Optionally filter to one exact value. Switches to the page of the first match; matches on other pages are counted but not selected, so page through with pageName or offset. Use this instead of get_variable_bindings when the goal is 'show me those nodes' rather than 'give me the numbers'.",
  {
    ...
    value: import_zod.z.union([import_zod.z.string(), import_zod.z.number()]).optional().describe('Only match this exact value, e.g. 100 for cornerRadius or "#2974ff" for fills. Omit to match any hardcoded value'),
    pageName: import_zod.z.string().optional().describe("Limit the scan to a single page by name"),
    nodeId: import_zod.z.string().optional().describe("Limit the scan to this node and its children"),
    excludePages: import_zod.z.array(import_zod.z.string()).optional().describe("Page names to skip"),
    offset: import_zod.z.number().optional().describe("Matches to skip, for paging (default 0)"),
    limit: import_zod.z.number().optional().describe("Max nodes to select in one call (default 50)")
  },
  async (params) => {
    try {
