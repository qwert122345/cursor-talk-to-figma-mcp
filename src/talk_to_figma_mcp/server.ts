...
const tableCellSchema = z.object({
  ...
  style: z
    .string()
    .optional()
    .describe("Font style, e.g. 'Regular', 'Medium', 'Bold'. Default Regular"),
...
server.tool(
  ...
  "Build whole table rows (row frame + text cells + optional checkbox result column + divider) in one call. Use instead of creating each cell node separately: cell-by-cell creation costs ~17 round trips per row and risks dropping the websocket mid-build.",
  {
    parentId: z
      .string()
      .describe("Container to append rows to. Should be a VERTICAL auto-layout frame."),
    rows: z
      .array(
        z.object({
          ...
          fill: z
            ...
            .optional()
            .describe("Row background. Defaults to white"),
          result: z
            .object({
            ...
            .optional()
            .describe("Trailing per-round result column"),
          divider: z
            .boolean()
            .optional()
            .describe("Append a 1px divider after this row (default true)"),
        })
      )
  ...
  async (params: any) => {
    try {
      const result = (await sendCommandToFigma("create_table_rows", params)) as {
        rowCount: number;
        rows: Array<{ id: string; name: string }>;
        fontFallbacks?: string[];
      };
      const fallback =
        result.fontFallbacks && result.fontFallbacks.length
      ...
      return {
        content: [
          {
            type: "text",
            text: `Created ${result.rowCount} rows: ${result.rows
              .map((r) => `${r.name}=${r.id}`)
              .join(", ")}${fallback}`,
          },
        ],
