...
server.tool(
  ...
  async ({ x, y, text, fontSize, fontWeight, fontColor, name, parentId, fontFamily, fontStyle, width }: any) => {
    try {
      ...
      const typedResult = result as {
        ...
        fontFallbackReason?: string | null;
      };
      // Report the font that actually landed: a silent fallback to Inter is the
      // failure mode that is easy to miss until the whole page is built.
      const font = typedResult.fontName
        ? ` [font: ${typedResult.fontName.family} ${typedResult.fontName.style}]`
        : "";
      const fallback = typedResult.fontFallbackReason
        ? ` (FELL BACK to Inter: ${typedResult.fontFallbackReason})`
        : "";
      return {
        content: [
          {
...
server.tool(
  ...
  async ({ nodeIds, pageName, nodeId, excludePages, onlyOutsideComponents, property, properties, dryRun }: any) => {
    try {
...
// DS healthcheck: bulk opacity (e.g. dim finished checklist rows)
server.tool(
  ...
  async ({ nodeIds, opacity }: any) => {
    try {
      const result = await sendCommandToFigma("set_multiple_opacity", { nodeIds, opacity }, 60000);
      return { content: [{ type: "text", text: JSON.stringify(result) }] };
    } catch (error) {
      return {
        content: [
...
server.tool(
  ...
  async ({ nodeId, pageName, excludePages, nameFilter, includeFaded, limit, offset }: any) => {
    try {
      const result = await sendCommandToFigma(
        "find_hidden_nodes",
...
server.tool(
  ...
  "Count instances per main component across the current file, split into library (design system) and local components. Run it on a product file to measure design system usage. Pass instancesFor (component key/id/name) to also get where those instances are - id, page and ancestor path - which is the only way to locate instances whose layer name is the slot name rather than the component name.",
  {
    ...
    instancesFor: z
      .string()
      .optional()
      .describe("Component key, id, name or set name: also list where those instances actually are (id, page, ancestor path)"),
    instanceLimit: z.number().optional().describe("Max instances to list for instancesFor (default 1000)"),
  },
  async ({ pageName, excludePages, instancesFor, instanceLimit }: any) => {
    try {
      const result = await sendCommandToFigma(
        "get_instance_census",
...
server.tool(
  ...
  async ({ nodeIds }: any) => {
    try {
      ...
      const typedResult = result as { selectedNodes: Array<{ name: string; id: string }>; count: number; message?: string };
      return {
        content: [
          {
            type: "text",
...
// E-14: select the nodes that hardcode a property, so a human can review them in the file
server.tool(
  "select_hardcoded",
  "Find nodes that hardcode a property (fills, strokes, cornerRadius, itemSpacing, paddingLeft/Right/Top/Bottom, fontSize) instead of binding a variable, and SELECT them in Figma so a person can review them. Optionally filter to one exact value. Switches to the page of the first match; matches on other pages are counted but not selected, so page through with pageName or offset. Use this instead of get_variable_bindings when the goal is 'show me those nodes' rather than 'give me the numbers'.",
  {
    ...
    value: z
      ...
      .optional()
      .describe("Only match this exact value, e.g. 100 for cornerRadius or \"#2974ff\" for fills. Omit to match any hardcoded value"),
    pageName: z.string().optional().describe("Limit the scan to a single page by name"),
    nodeId: z.string().optional().describe("Limit the scan to this node and its children"),
    excludePages: z.array(z.string()).optional().describe("Page names to skip"),
...
type CommandParams = {
  ...
  grid_layout: {
    ...
    nodeId?: string;
    limit?: number;
    items?: { nodeId: string; props?: Record<string, unknown>; position?: Record<string, unknown> }[];
  };
  create_section: {
    x: number;
    y: number;
...
server.tool(
  ...
  "Read and write GRID auto-layout cell placement. IMPORTANT: a child's gridRowAnchorIndex / gridColumnAnchorIndex are READ-ONLY, so op:\"place\" (which calls the parent frame's setGridChildPosition) is the only way to move a cell; op:\"set\" is for the frame's own gridRowCount / gridRowSizes. Needed because cloning a cell inside a GRID frame makes the clone inherit the source's placement, so it lands stacked on the original and the table gains no row. op:\"info\" dumps the frame's grid properties, every child's placement, and a `discovered` list of the grid-related property/method names that actually exist on this Figma version — use it before writing. op:\"set\" assigns the grid* properties you pass (row/column anchor and span on children, row/column count on the frame); non-grid keys are refused. Deleting cells and replacing cell text already work without this tool.",
  {
    op: z
      .enum(["info", "set", "place"])
      .describe('"info" to inspect, "set" to assign grid* properties, "place" to position a child in the grid'),
    ...
    items: z
      ...
      .optional()
      .describe("set/place: entries to apply, each reported independently"),
  },
  async ({ op, nodeId, limit, items }: any) => {
    try {
